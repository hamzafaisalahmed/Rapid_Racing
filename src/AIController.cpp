...
void AIController::updateSteering()
{
    ...
    // static sf::Clock steerDbgTimer;
    // if (steerDbgTimer.getElapsedTime().asSeconds() > 0.15f)
    // {
    //     std::cout << "[STEER] currIdx=" << currIdx
    //               << " angleDeg=" << angleDeg
    //               << " horizInput=" << (int)horizontalInput
    //               << " carPos=(" << car->getPosition().x << "," << car->getPosition().y << ")\n";
    //     steerDbgTimer.restart();
    // }
...
void AIController::updateSpeed()
{
    ...
    // static sf::Clock speedDbgTimer;
    // if (speedDbgTimer.getElapsedTime().asSeconds() > 0.15f)
    // {
    //     std::cout << "[SPEED] currIdx=" << currIdx
    //               << " effectiveZone=" << effectiveZone
    //               << " targetSpeed=" << targetSpeed
    //               << " currSpeed=" << currSpeed
    //               << " distToCornerStart=" << distToCornerStart
    //               << " laneBrakeDist=" << laneBrakeDist << "\n";
    //     speedDbgTimer.restart();
    // }
    if (trafficSpeedCap >= 0.f)
    {
        targetSpeed = std::min(targetSpeed, trafficSpeedCap);
        // if (targetSpeed == trafficSpeedCap)
        //     std::cout << "IM FOLLOWING THE TRAFFIC SPEED RN" << std::endl
        //               << std::endl;
    }
...
//     float halfWidth = magnitude(frontRight - frontLeft) * 0.5f;
...
//         if (other == car)
//             continue;
//         if (state == AIState::Passing && other == passingRival && isLaneClear(currentLane, leaderboard))
//             continue;
//         sf::Vector2f toP = other->getPosition() - frontMid;
...
//         {
//             if (alongDist < closestDist)
//             {
//                 closestDist = alongDist;
//                 carAhead = other; // was missing
//             }
//         }
//     }
...
//     if (carAhead)
//     {
//         // Only the speed cap is conditionally suppressed here — carAhead itself
//         // stays set whenever a car is physically in the projection box, so
//         // `blocked` in updateState() remains accurate and the FSM doesn't get
//         // yanked into Cruising just because we're choosing not to cap speed.
//         bool exemptFromCap = (state == AIState::Passing && carAhead == passingRival && isLaneClear(currentLane, leaderboard));

//         if (!exemptFromCap)
//             trafficSpeedCap = carAhead->getCurrSpeed();
//         // else: trafficSpeedCap stays -1.f (no cap), carAhead stays set (still "blocked")
//     }
// }
void AIController::updateTrafficAwareness(const std::vector<Car *> &leaderboard)
{
...
//     float laneLateralOffset = dotProduct(laneTarget - car->getPosition(), perp);
...
//         sf::Vector2f toOther = other->getPosition() - car->getPosition();
//         float alongDist = dotProduct(toOther, heading);
//         if (std::abs(alongDist) > sideCorridorHalfLen)
//             continue; // not near enough fwd/back

//         float otherLateral = dotProduct(toOther, perp);

//         float combinedHalfWidth = (car->getDimensions().x + other->getDimensions().x) * 0.5f;
//         if (std::abs(otherLateral - laneLateralOffset) < combinedHalfWidth * 1.2f)
//             return false; // occupies the lane
//     }
//     return true;
// }
bool AIController::isLaneClear(TargetSide lane, const std::vector<Car *> &leaderboard) const
...
//     if (state == AIState::Passing && carAhead != passingRival)
//     {
//         state = AIState::Following;
//         passingRival = nullptr;
//         followingHoldTime = 0.f; // reset hold on entry to Following
//     }
...
//     {
//         if (hasPassed(passingRival, leaderboard))
//         {
//             state = AIState::Cruising;
//             passingRival = nullptr;
//             return;
...
//         {
//             laneBlockedTime += dt;
//             if (laneBlockedTime > laneBlockedTimeThreshold)
//             {
//                 state = AIState::Following;
//                 passingRival = nullptr;
//                 laneBlockedTime = 0.f;
...
//         if (idxDelta >= maxPassingIndexSpan)
//         {
//             state = AIState::Following;
//             passingRival = nullptr;
//             followingHoldTime = 0.f; // reset hold on entry to Following
...
//         if (movedDist < 5.f) // essentially stationary this frame
//             passingStuckTime += dt;
//         else
//         {
//             passingStuckTime = 0.f;
...
//         if (passingStuckTime > maxPassingStuckTime)
//         {
//             state = AIState::Following;
//             passingRival = nullptr;
//             followingHoldTime = 0.f; // reset hold on entry to Following
